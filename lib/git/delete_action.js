...
var runDelete = function *(eventData, cbk) {
  ...
  if (process.env.OVERRIDE_REPO_URL) {
       var toReplace = eventData.repourl.split(':')[0];
       var overrideRepoUrl = eventData.repourl.replace(toReplace, process.env.OVERRIDE_REPO_URL);
  } else {
       var overrideRepoUrl = eventData.repourl;
  }
  logger.debug("Delete data are : "+overrideRepoUrl);
  // var rundeck_data = { "pushUsername": pushUsername, "branch": puppetfileBranchName, "module": moduleName };
  ...
  var branch_regex_builder = '([-\\w]+)\\.git[\'"],\\s*:ref\\s*=>\\s*[\'"]'+puppetfileBranchName+'[\'"]$';
  var module_regex_builder = '('+moduleName+'\\.git[\'"],\\s*:ref\\s*=>\\s*[\'"])'+puppetfileBranchName+'([\'"]\\s*)$';
  ...
  try {
    ...
    logger.info('Entering "delete_action" process for branch '+puppetfileBranchName+' of module '+moduleName);
    ...
    */
    ...
    if (!branchExist) {
      ...
      logger.info('Someone seems to have already (maybe manually?) deleted branch '+puppetfileBranchName+' on Puppetfile repo. Nothing to do.');
      return cbk(null);
    }
    ...
    */
    ...
    logger.info('Checking if module '+moduleName+' is the last one referencing branch "'+puppetfileBranchName+'"');
    ...
    */
    if (module_list.length == 0) {
      ...
      logger.info('This is the last module in the branch '+puppetfileBranchName+', we can delete the branch from the Puppetfile repo');
      yield* GU.del(puppetfileBranchName, gitopts);
    ...
    rd_job_status ? logger.info('Processing branch '+puppetfileBranchName+' of module '+moduleName+' '+rd_job_status) : logger.info('Branch '+puppetfileBranchName+' of module '+moduleName+' successfully processed');
