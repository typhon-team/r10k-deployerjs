...
var runCreate = function *(eventData, cbk) {
  ...
  logger.debug("Create data are : "+overrideRepoUrl);
  ...
  var module_regex = new RegExp('('+overrideRepoUrl+'[\'"],\\s*:ref\\s*=>\\s*[\'"])([^\'"]+)([\'"]\\s*)$', 'gm');
  logger.debug("Created data are : "+overrideRepoUrl);

  try {

    logger.info('Entering "create_action" process for branch '+puppetfileBranchName+' of module '+moduleName);
