...
var runJob = function *(deployInfo) {
  ...
  var job_args = '-push_user "'+deployInfo.pushUsername+'" -r10k_type "'+deployInfo.type+'" -r10k_env "'+deployInfo.branch+'" -r10k_module "'+deployInfo.module+'" -git_module_dest "' +deployInfo.path+ '" -git_module_ref "' + deployInfo.repoUrl+'"'; 
  var job_status;
  var req_params;
  var response;
  ...
  logger.debug('Rundeck params: '+req_params.form.argString);
  ...
  if (response.statusCode != '200') throw new Error('API call returned a '+response.statusCode+' when trying to launch Rundeck job :'+response.body);
